...
local ESP = {
	...
	TracerColor = Color3.fromRGB(255, 0, 4),
	FaceCamera = false,
	Names = false,
	Distances = false,
	TeamColor = true,
...
function ESP:GetColor(obj)
	...
	return p and self.TeamColor and p.Team and p.Team.TeamColor.Color
end
...
function boxBase:Update()
	...
	local ESPColor = ESP:GetColor(self.Object) or ESP.ESPColor
	local TracerColor = ESP:GetColor(self.Object) or ESP.TracerColor

	local allow = true
	if ESP.Overrides.UpdateAllow and not ESP.Overrides.UpdateAllow(self) then
		allow = false
	end
	if self.Player and ESP.IgnoreTeam and ESP:IsTeamMate(self.Player) then
		allow = false
	end
...
function ESP:Add(obj, options)
	...
	box.Components["Quad"] = Draw("Quad", {
		Thickness = ESP.ESPThickness,
		Color = box.Color,
		Transparency = 1,
		Filled = false,
		Visible = ESP.ESPEnabled and self.Boxes
	})
